...
Y_train = np.loadtxt('dataset/cat_train_y.csv', delimiter=',').reshape(1, -1)
X_test = np.loadtxt('dataset/cat_test_x.csv', delimiter=',') / 255.0
Y_test = np.loadtxt('dataset/cat_test_y.csv', delimiter=',').reshape(1, -1)
...
def compute_cost(AL, Y):
    ...
    cost = np.squeeze(cost)  # Returns a single digit. [[7]] will return 7
    return cost
